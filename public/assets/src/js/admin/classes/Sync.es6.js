...
class Sync
{
    ...
    async show_posts( process_id )
    {
        ...
        if ( 'undefined' != typeof process.data.source_data.posts ) {
            // BULK REFRESH
            for ( const [ index, post ] of process.data.source_data.posts.entries() ) {
                ...
                await this.get_syndicated_post_details(post.ID, process.data).
                    then(retrieved_post => syndicated_posts.display_refreshed_post(
                        retrieved_post,
                        process_id));
            }
        }
    ...
    async get_source_data( process_id )
    {
        ...
        let source_post_id = ( false === process.source_post_id )
            ? 0
            : process.source_post_id;

        const response = await fetch(DataSync.api.url + '/source_data/load/' +
            source_post_id); // LOAD BULK
    ...
    get_receiver_data( process_id )
    {
        ...
        process.connected_sites.forEach(site => {
            receiver_data_requests.push(fetch(site.api_url +
                'data-sync/v1/receiver/get_data'));
        });
    ...
    async get_posts( process_id )
    {
        ...
        if ( false === process.source_post_id ) {
            ...
            const response = await fetch(DataSync.api.url + '/posts/' +
                process.source_post_id);
            return await response.json().
                catch(message => Message.handle_error(message, process));
        }
            ...
            then(prevalidation => {
                if ( !prevalidation.success ) {
                    ...
                    this.send_posts_to_receivers(process.id).
                        ...
                        .then(( process ) => Logs.process_receiver_logs(process.receiver_data,
                            process.id,
                            process.topic)).
                        then(() => this.process_receiver_synced_posts(process.id)).
                        then(() => this.consolidate_media(process.id)).
                        then(( consolidated_media_packages ) => this.send_media_to_receivers(
                            consolidated_media_packages,
                            process.id)).
                        then(() => {return Processes.get(process.id);}) // refresh data
                        .then(( process ) => Logs.process_receiver_logs(process.media_sync_responses,
                            process.id,
                            process.topic)).
                        then(() => this.process_receiver_synced_posts(process.id)).
    ...
    consolidate( process_id )
    {
        ...
        return this.get_posts(process_id).then(posts => {
        ...
        return Promise.all(requests) // send all requests for package
            ...
            .then(prepared_site_packages => {
                ...
                prepared_site_packages.forEach(site_packages => {
                    site_packages.forEach(media_package => {
                        consolidated_packages.push(media_package);
                    });
                });
                ...
                admin_message.topic = process.topic;
                admin_message.success = true;
                admin_message.message = 'Media packages ready. <span id="process_' +
                    process.id + '">0</span>/' + consolidated_packages.length +
                    ' media items synced.';
                Message.admin_message(admin_message);
                return consolidated_packages;
            });
    ...
    send_posts_to_receivers( process_id )
    {
        ...
        let process = Processes.get(process_id);
        process.prepped_source_packages.forEach(prepped_source_package => this.create_remote_request(
            prepped_source_package,
            process_id)); // create send requests with packages
    ...
    prepare_packages( process_id )
    {
        ...
        if ( false === process.receiver_site_id ) {
            process.connected_sites.forEach(site => {
                if ( false === process.source_post_id ) {
                    requests.push(fetch(DataSync.api.url +
                        '/source_data/prep/0/' + site.id));
                } else {
                    requests.push(fetch(DataSync.api.url +
                        '/source_data/prep/' + process.source_post_id + '/' +
                        site.id));
                }
            });
        } else {
            requests.push(fetch(DataSync.api.url + '/source_data/prep/' +
                process.source_post_id + '/' + process.receiver_site_id));
        }
    ...
    prepare_media_packages( process_id )
    {
        ...
        process.connected_sites.forEach(site => {
            if ( ( false === process.receiver_site_id ) ||
                ( parseInt(site.id) === process.receiver_site_id ) ) {
                process.prepped_source_packages.forEach(prepped_source_package => {
                    ...
                    if ( parseInt(site.id) ===
                        parseInt(decoded_package.receiver_site_id) ) {
                        data.site = site;
    ...
    async send_media( media_package, process_id )
    {
        ...
        let process = Processes.get(process_id);
        let i = process.connected_sites.findIndex(site => site.id ==
            source_package.receiver_site_id);

        const response = await fetch(process.connected_sites[ i ].api_url +
            'data-sync/v1/sync', {
    ...
    process_receiver_synced_posts( process_id )
    {
        ...
        process.receiver_data.forEach(single_receiver_data => receiver_synced_posts.push(
            single_receiver_data.data.synced_posts));
    ...
    prevalidate_receivers( process_id )
    {
        ...
        process.connected_sites.forEach(site => {
            receiver_prevalidation_requests.push(fetch(site.api_url +
                'data-sync/v1/receiver/prevalidate'));
        });
    ...
    create_remote_request( prepped_source_package, process_id )
    {
        ...
        const i = process.connected_sites.findIndex(site => site.id ==
            source_package.receiver_site_id);

        process.receiver_sync_requests.push(fetch(process.connected_sites[ i ].api_url +
            'data-sync/v1/sync', {
            method: 'POST', body: JSON.stringify(source_package),
        }));
