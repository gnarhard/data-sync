...
class SyndicatedPosts
{
    ...
    init()
    {
        ...
        if ( document.getElementById('bulk_data_push') ) {
            document.getElementById('bulk_data_push').onclick = function( e ) {
                ...
                document.querySelector('#syndicated_posts_wrap .loading_spinner').
                    classList.
                    remove('hidden'); // SHOW
                // LOADING
                // SPINNER.
        ...
        ConnectedSites.get_all(process.id).
            ...
            then(source_data => {
                ...
                if ( !source_data.success ) {
                    ...
                    admin_message.message = source_data.data.error_msg;
    ...
    display_refreshed_post( retrieved_post, process_id )
    {
        ...
        if ( false === process.source_post_id ) {
            ...
            $('#synced_post-' + process.source_post_id).
                addClass('flash_success');
        }
    ...
    single_post_actions_init()
    {
        ...
        $('.expand_post_details').
            unbind().
            click(e => $('#post-' + e.target.dataset.id).toggle());
        ...
        $('.overwrite_single_receiver').unbind().click(e => {
            ...
            processes.forEach(process => {
                if ( ( source_post_id === parseInt(process.source_post_id) ) &&
                    ( receiver_site_id ===
                        parseInt(process.receiver_site_id) ) ) {
                    if ( 'push_post_now_' + source_post_id ===
                        process.button_id ) {
                        let admin_message = {};
                        admin_message.process_id = process.id;
                        admin_message.success = false;
            ...
            if ( create_new_process ) {
                ...
                let process = {
                    ...
                    topic: 'Post ' + source_post_id + ', Receiver ' +
                        receiver_site_id,
                    running: true,
