...
class Options {
	...
	public function admin_menu() {
		add_menu_page( 'Data Sync', 'Data Sync', 'manage_options', 'data-sync-options',
			$this->view_namespace . '\data_sync_options_page', 'dashicons-networking' );
	}
	...
	public static function prevalidate( $receiver_prevalidation_data ) {
		...
		$prevalidation_data->source->acf   = ( is_plugin_active( 'advanced-custom-fields-pro/acf.php' ) );
		$prevalidation_data->source->cptui = ( is_plugin_active( 'custom-post-type-ui/custom-post-type-ui.php' ) );
		$prevalidation_data->source->yoast = ( is_plugin_active( 'wordpress-seo/wp-seo.php' ) );

		$plugins = get_plugins();

		$source_acf_version   = $plugins['advanced-custom-fields-pro/acf.php']['Version'] ?? false;
		$source_cptui_version = $plugins['custom-post-type-ui/custom-post-type-ui.php']['Version'] ?? false;
		$source_yoast_version = $plugins['wordpress-seo/wp-seo.php']['Version'] ?? false;
		...
		if ( ! is_wp_error( $receiver_prevalidation_data ) ) {
			...
			foreach ( $receiver_prevalidation_data as $site_plugin_data ) {
				...
				$prevalidation_data->receiver[ $index ]->wp_version_synced    = ( $source_wp_version === $site_plugin_data->versions->wp );
				$prevalidation_data->receiver[ $index ]->acf_version_synced   = ( $source_acf_version === $site_plugin_data->versions->acf );
				$prevalidation_data->receiver[ $index ]->cptui_version_synced = ( $source_cptui_version === $site_plugin_data->versions->cptui );
				$prevalidation_data->receiver[ $index ]->yoast_version_synced = ( $source_yoast_version === $site_plugin_data->versions->yoast );
	...
	public static function validate_required_plugins_info( $site, $prevalidation_data ) {
		...
		if ( ! $prevalidation_data->source->acf ) {
			$message .= 'ACF needs to be installed or activated on this site. ';
		}

		if ( ! $prevalidation_data->source->cptui ) {
			$message .= 'CPTUI needs to be installed or activated on this site. ';
		}

		if ( ! $prevalidation_data->source->yoast ) {
			$message .= 'Yoast SEO needs to be installed or activated on this site. ';
		}
		...
		foreach ( $prevalidation_data->receiver as $receiver_plugin_info ) {
			if ( (int) $site->id === $receiver_plugin_info->site_id ) {
				...
				if ( ! $receiver_plugin_info->cptui_version_synced ) {
					$message .= 'CPTUI\'s plugin version is different on <a target="_blank" href="' . $receiver_plugin_info->data->site_admin_url . '">' . $site->name . '</a>. Update receiver or source site to resolve this error. ';
				}

				if ( ! $receiver_plugin_info->acf_version_synced ) {
					$message .= 'ACF\'s plugin version is different on <a target="_blank" href="' . $receiver_plugin_info->data->site_admin_url . '">' . $site->name . '</a>. Update receiver or source site to resolve this error. ';
				}

				if ( ! $receiver_plugin_info->wp_version_synced ) {
					$message .= 'The WordPress core version is different on <a target="_blank" href="' . $receiver_plugin_info->data->site_admin_url . '">' . $site->name . '</a>. Update receiver or source site to resolve this error. ';
				}

				if ( ! $receiver_plugin_info->yoast_version_synced ) {
					$message .= 'Yoast\'s plugin version is different on <a target="_blank" href="' . $receiver_plugin_info->data->site_admin_url . '">' . $site->name . '</a>. Update receiver or source site to resolve this error. ';
				}
			}
		}
	...
	public function register() {
		...
		add_settings_field( 'source_site', 'Source or Receiver?', $this->view_namespace . '\display_source_input',
			'data-sync-options', 'data_sync_options' );
		register_setting( 'data_sync_options', 'source_site' );

		register_setting( 'data_sync_options', 'debug' );

		add_settings_field( 'awareness_messages', '', $this->view_namespace . '\awareness_messages',
			'data-sync-options', 'data_sync_options' );
		...
		if ( '1' === $source ) :
			...
			add_settings_field( 'enable_new_cpts', 'Automatically Enable New Custom Post Types On Receiver',
				$this->view_namespace . '\display_auto_add_cpt_checkbox', 'data-sync-options', 'data_sync_options' );
			register_setting( 'data_sync_options', 'enable_new_cpts' );

			add_settings_field( 'overwrite_receiver_post_on_conflict',
				'Overwrite Receiver Post if Receiver Post Was More Recently Edited',
				$this->view_namespace . '\display_overwrite_receiver_post_checkbox', 'data-sync-options',
				'data_sync_options' );
			register_setting( 'data_sync_options', 'overwrite_receiver_post_on_conflict' );

			add_settings_field( 'debug', 'Debug', $this->view_namespace . '\display_debug_checkbox',
				'data-sync-options', 'data_sync_options' );

			if ( '1' === get_option( 'debug' ) ) :

				add_settings_field( 'start_fresh', 'Start Fresh', $this->view_namespace . '\display_start_fresh_link',
					'data-sync-options', 'data_sync_options' );
			endif;
		elseif ( '0' === $source ) :

			add_settings_field( 'secret_key', 'Secret Key', $this->view_namespace . '\display_secret_key',
				'data-sync-options', 'data_sync_options' );
			register_setting( 'data_sync_options', 'secret_key' );

			add_settings_field( 'notified_users', 'Notified Users', $this->view_namespace . '\display_notified_users',
				'data-sync-options', 'data_sync_options' );
			register_setting( 'data_sync_options', 'notified_users' );

			register_setting( 'data_sync_options', 'enabled_post_types' );
			add_settings_field( 'enabled_post_types', 'Enabled Post Types',
				$this->view_namespace . '\display_post_types_to_accept', 'data-sync-options', 'data_sync_options' );
